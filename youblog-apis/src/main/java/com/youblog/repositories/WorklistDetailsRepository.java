...
public interface WorklistDetailsRepository extends JpaRepository<WorklistDetailsEntity, Long> {
	...
	@Query(value = "select wd.*,concat(ud.first_name,' ',ud.last_name) as username,ud.email_id,\r\n"
			+ "ud.role_id,ud.category_id,ud.image_id,ud.active_flag,wm.workflow_name,ud.gym_id from worklist_details wd \r\n"
			...
			+ "where wd.worklist_status = :status and wd.action_user_id = :userId order by wd.initiated_date desc", nativeQuery = true)
	...
	@Query(value = "select wd.*,concat(ud.first_name,' ',ud.last_name) as username,ud.email_id,\r\n"
			...
			+ "inner join workflow_master wm on wm.workflow_master_id = wd.workflow_master_id \r\n"
			+ "where wd.initiated_user_id = :userId order by wd.initiated_date desc", nativeQuery = true)
	public List<Object[]> getRequestedWorklistData(Long userId);

	@Query(value = "select wd.*,concat(ud.first_name,' ',ud.last_name) as username,ud.email_id,\r\n"
			+ "ud.role_id,ud.category_id,ud.image_id,ud.active_flag,wm.workflow_name from worklist_details wd \r\n"
			+ "inner join user_details ud on ud.user_id = wd.action_user_id \r\n"
			+ "inner join workflow_master wm on wm.workflow_master_id = wd.workflow_master_id \r\n"
			+ "where wd.worklist_status = :status and wd.initiated_user_id = :userId and wd.workflow_master_id = :workflowMasterId order by wd.initiated_date desc", nativeQuery = true)
