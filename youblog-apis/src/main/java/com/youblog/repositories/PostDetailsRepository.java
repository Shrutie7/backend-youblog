...
public interface PostDetailsRepository extends JpaRepository<PostDetailsEntity, Long>{
	...
	@Query(value = "select pd.post_id,pd.title,pd.content,pd.user_id,pd.category_id,to_char(pd.created_date,'dd Mon yy HH24:MI') as createdDate,pd.decription,\r\n"
			...
			+ "uud.role_id as urole,uud.location_id as uloc,likeCount.likes,case when likeStatus.post_id is null then false else true end as like_status from post_details pd \r\n"
			...
			+ "post_likes pl group by post_id)likeCount on likeCount.post_id = pd.post_id\r\n"
			...
			+ "where pd.active_flag = true and pd.archive_flag=false and pd.category_id = :categoryId and pd.user_id in \r\n"
			...
			+ "select distinct(user_id) from userids) order by pd.updated_date desc",nativeQuery = true)
